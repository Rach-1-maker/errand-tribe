      ...
      <main className='grow relative'>
        ...
        <section className='grid grid-cols-1 md:grid-cols-2 gap-8 items-center md:px-20 py-10 md:py-20 md:mx-10 relative'>
          ...
          <div className='relative z-20 flex flex-col sm:flex-row gap-4 justify-center md:justify-start md:gap-8'>
            <button onClick={openModal} className='inline-block text-center bg-white text-[#424BE0] px-6 py-3 rounded-2xl font-medium hover:border-2 hover:border-white hover:text-white hover:bg-transparent transition duration-500 cursor-pointer'>
            Post Errands
            </button>
            <button onClick={openModal} className='inline-flex items-center justify-center gap-2 border-2 border-white bg-transparent transition duration-500  px-6 py-3 rounded-2xl cursor-pointer'>
            Join as Runner <FaArrowRightLong />
            </button>
          </div>
          </div>
          {/* Image section */}
